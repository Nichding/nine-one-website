...
## 2026-07-11 · 官网 CI V2.0 全量迁移（Phase A–D）

- **现行唯一基准**：Brand CI V2.0，29-token（8 neutral + Jade / Azure / Violet 各 7 阶）。V1 旧 6-token 口径已作废。
- **页面语境**：Home / Ecosystem / About / Build = Master；Play / Play Specs = Jade；Technology = Azure；Facility / Air Dome = Violet。业务页 CTA 与深色页脚使用本族 hero / night 配对。
- **字号例外**：官网保留 V1 期已定稿的 display 字阶；原因是现有产品页 display 体系已完成响应式定稿；**失效日期：下次 CI 修订**。本轮未新增字重。
- **治理引用**：所有对外发布改按 **V2 Pre-Publish QA Gate（6.1 九项）** 验收，不再使用旧 V2 Pre-Publish QA Gate（6.1 九项） 口径。
- **资产治理**：新增 `assets/images/manifest.json`，登记 `assets/images/opt/` 全部图片并标注 `ai-concept` / `reference`。
- **明确遗留**：padel 规格表数值待供应商目录核对；Play OS / Smart 表述待页面方向确定。
- 四个 Phase 均仅提交到 `redesign/ci-v2-phase2`；未 push、未 merge main、未创建或处理 PR。
...
- **T2 · Play 能力详情**（commit `d0810e8`）：6 个 `.capplus` 已接同一个 overlay modal；支持 Esc、点击遮罩关闭、关闭后焦点返回、移动端全屏。六段英文详情初稿各 80–85 词，页面内明确标注“待 V2 Pre-Publish QA Gate（6.1 九项） + Nicholas 签字”。
...
1. 六段能力详情英文初稿通过 V2 Pre-Publish QA Gate（6.1 九项） 并签字。
...
8 页官网已整体重做成 **Apple 式产品生态站**（Inter + Spline Sans Mono、CI V2.0 29-token、白底为主、零非 CI hex）。**AI 配图系统已开跑**：Play 页四条产品线（Kauri / Kawarau / Tekapo / Rangitoto）已全部用上 Gemini 概念渲染图（原档在本文件夹 `Imagery/Renders/`）。之前「7 页未提交」已于 6-28 提交（`751e6cf`）；当前仅 sports.html 的 Rangitoto 段落 + 新图未提交。**线上跑的仍是旧版，本轮重做未上线。**
...
- 待办：媒体框换真图/视频（prompt 待出）→ specs 其余 `[__]` 填真值 → 计算器接逻辑（敏感成本系数不进前端）→ V2 Pre-Publish QA Gate（6.1 九项） → 并入正式 facility.html 或替换。
...
- 自测已过：零非 CI V2 token、390/768/1440 无横向滚动、图片正常加载。
...
- [ ] **能力卡「+」详情页/弹层**：6 个「+」当前无动作，待建（文案量最大，先出草稿过 V2 Pre-Publish QA Gate（6.1 九项））。
...
- **文案签字**：新增对外文案（Designed for New Zealand、各 hero、Facility 等）属对外产出，发布前须过 **V2 Pre-Publish QA Gate（6.1 九项） 合规清单** + Nicholas 过目。
...
4. 文案签字 → V2 Pre-Publish QA Gate（6.1 九项） 合规 → 等 Nicholas 说「上线」再合并 `main` 部署。
...
- **CI 正本**：`2_NineOneSports/0_Company/Brand_CI/NINEONE_CI_System.html`（发布前过 V2 Pre-Publish QA Gate（6.1 九项））
- **29 token**：唯一正源为 `Brand_CI/manifest/tokens.json` 与 CI V2.0；网站不得使用清单外色值。
...
- `home-concept.html` 已完成 problem-led 文案闭环更新，待 Nicholas 签字后转正替换 `index.html`。
